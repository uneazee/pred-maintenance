"""
Model Comparison Analysis - JOBLIB VERSION (Fixed for your models)
Run from workspace root: python analyze_models_joblib.py

This version uses joblib.load() instead of pickle.load()
which is the correct method for scikit-learn models.
"""
...
def load_models(models_dir='models'):
    """Load all trained models and scaler using joblib."""
    ...
    if metadata_path.exists():
        metadata = joblib.load(metadata_path)  # ← Using joblib
    ...
    if scaler_path.exists():
        scaler = joblib.load(scaler_path)  # ← Using joblib
    ...
    for name, filename in model_files.items():
        ...
        if model_path.exists():
            models[name] = joblib.load(model_path)  # ← Using joblib
            print(f"✓ Loaded {name}")
        else:
...
def plot_precision_recall_curves(results, y_test, save_path=None):
...
def plot_feature_importance(models, save_path=None):
    ...
    for idx, (name, model) in enumerate(tree_models.items()):
        ...
        axes[idx].set_yticklabels([f'Feature {i}' for i in indices], fontsize=8)
...
def main():
    ...
    if not Path(MODELS_DIR).exists():
        ...
        print("   python analyze_models_joblib.py")
        return
    ...
    if not X_test_path or not y_test_path:
        ...
        print()
        print("Models loaded successfully, but test data is needed for analysis.")
        print()
        print("To generate test data, add these lines to train_model.py (after line 422):")
        print()
    ...
    plot_precision_recall_curves(results, y_test, output_path / 'pr_curves.png')
    plot_metrics_comparison(comparison_df, output_path / 'metrics_comparison.png')
    plot_feature_importance(models, output_path / 'feature_importance.png')
